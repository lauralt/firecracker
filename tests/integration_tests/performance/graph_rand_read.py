...
for job in jobs:
    if job.startswith("{'fio version': 'fio-3.1'"):
        ...
        if job_results[0]["job options"]["rw"] == "randread":
...
plt.title('Fio results for random reads for different block sizes', loc='center')
...
# plt.title('Fio results for random reads for different block sizes (vsock)', loc='center')
# plt.xlabel('Block sizes [kB]')
# plt.ylabel('Bandwidth [kB/sec]')
# plt.legend()
#
