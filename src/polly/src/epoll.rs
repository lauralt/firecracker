...
pub enum ControlOperation {
    /// Adds a file descriptor to the interest list.
    EPOLL_CTL_ADD = EPOLL_CTL_ADD,
    /// Changes the settings associated with a file descriptor that is
...
impl EventType {
    /// Check if `self` event is an event that we are monitoring.
    pub fn is_contained_in(self, events: u32) -> bool {
...
#[repr(transparent)]
#[derive(Clone, Copy)]
pub struct Event(pub epoll_event);
...
impl Event {
    ...
    pub fn from_bits(events: u32) -> Option<u32> {
        let mask = EventType::EPOLL_IN as u32
            | EventType::EPOLL_OUT as u32
            | EventType::EPOLL_RDHUP as u32
            | EventType::EPOLL_ERR as u32;
        if events & !mask == 0 {
            return Some(events);
        }
        None
...
impl Epoll {
    ...
    /// * `event` is a pointer to an instance of `epoll_event` which keeps the event
    /// we want to monitor `fd` for.
    pub fn ctl(self, operation: ControlOperation, fd: RawFd, mut event: Event) -> io::Result<()> {
    ...
    /// # Arguments
    ///
    /// * `size` is the maximum number of events that can happen in the interest list.
    /// * `timeout` specifies for how long the `epoll_wait` system call will block
    /// (measured in milliseconds).
    pub fn wait(self, size: usize, timeout: i32) -> io::Result<Vec<Event>> {
        let mut buffer = vec![Event::empty(0); size];
