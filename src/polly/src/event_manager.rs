...
use epoll;
use pollable::{EpollConfig, EventRegistrationData, PollableOp};
...
impl EventManager {
    ...
    fn register_handler(
        ...
        self.epoll
            .ctl(epoll::ControlOperation::Add, fd, event_type.into())
            .map_err(Error::Poll)?;
    ...
    /// Update an event handler fds and event sets.
    /// Use the EpollConfig to build the vector of PollableOps.
    ///
    pub fn update(
        &mut self,
    ...
    fn update_event(&mut self, event: EventRegistrationData) -> Result<()> {
        if let Some(handler_data) = self.handlers.get_mut(&event.0) {
            self.epoll
                .ctl(epoll::ControlOperation::Modify, event.0, event.1.into())
                .map_err(Error::Poll)?;
    ...
    /// Unregister the event handler for the specified fd.
    ///
    pub fn unregister(&mut self, pollable: RawFd) -> Result<()> {
        match self.handlers.remove(&pollable) {
            Some(_) => {
                self.epoll
                    .ctl(epoll::ControlOperation::Delete, pollable, 0)
                    .map_err(Error::Poll)?;
            }
            None => {
    ...
    fn dispatch_event(
        ...
        if epoll::EventType::Error.has_match_in(evset) {
            ...
            // READ, WRITE, CLOSE.
            if epoll::EventType::Read.has_match_in(evset) {
                all_ops.append(&mut handler.handle_read(source));
            }
            if epoll::EventType::Write.has_match_in(evset) {
                all_ops.append(&mut handler.handle_write(source));
            }
            if epoll::EventType::ReadHangUp.has_match_in(evset) {
...
mod tests {
    ...
    fn test_event_type_to_epoll_mask() {
        let mask: u32 = (EventSet::READ | EventSet::WRITE | EventSet::CLOSE).into();
        let epoll_mask = epoll::EventType::Read as u32
            | epoll::EventType::Write as u32
            | epoll::EventType::ReadHangUp as u32;
