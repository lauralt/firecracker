...
fn main() {
    ...
    if no_api {
        ...
        let api_event_fd = event_fd.try_clone().expect("cannot clone Eventfd.");
        let api_shared_info = shared_info.clone();

        thread::Builder::new()
            .name("fc_api".to_owned())
            .spawn(move || {
                match ApiServer::new(mmds_info, shared_info, to_vmm, event_fd)
